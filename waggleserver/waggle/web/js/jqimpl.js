function jqReport(id,parent,params,width,height) {
	...
	for(let i=0;i<s;i++){
		...
		if(col.formatter=='image'){
			...
			let link='function imageFormat'+i+'(cellValue,options,rowObject){ if(cellValue=="Active" || cellValue=="Completed"){';
			link+=' return "<span class=\''+image[0]+'\'>"+cellValue+"</span>"; }else if(cellValue=="InActive" || cellValue=="Pending"){ return "<span class=\''+image[1]+'\'>"+cellValue+"</span>";}}';
			let b=eval("("+link+")");
			v.colModel[i].formatter=b;
		}
	}
	jQuery("#report_"+id).jqGrid(v);
	jQuery("#report_"+id).jqGrid('navGrid','#paging_'+id,{add:false,edit:false,del:false}, {}, {}, {}, {multipleSearch:true, multipleGroup:false, showQuery: false},{search: true});
	jQuery("#report_"+id).jqGrid('filterToolbar');

	$("#search_report_"+id+" span").removeClass("ui-icon ui-icon-search");
	$("#search_report_"+id+" span").addClass("ui-icon mdi mdi-file-find");
  
	$("#refresh_report_"+id+" span").removeClass("ui-icon ui-icon-refresh");
	$("#refresh_report_"+id+" span").addClass("ui-icon mdi mdi-refresh");

	$("#first_paging_"+id+" span").removeClass("ui-icon ui-icon-seek-first");
	$("#first_paging_"+id+" span").addClass("ui-icon mdi mdi-page-first");
	$("#prev_paging_"+id+" span").removeClass("ui-icon ui-icon-seek-prev");
	$("#prev_paging_"+id+" span").addClass("ui-icon mdi mdi-chevron-left");
	
	$("#next_paging_"+id+" span").removeClass("ui-icon ui-icon-seek-next");
	$("#next_paging_"+id+" span").addClass("ui-icon mdi mdi-chevron-right");
	$("#last_paging_"+id+" span").removeClass("ui-icon ui-icon-seek-end");
	$("#last_paging_"+id+" span").addClass("ui-icon mdi mdi-page-last");
...
function changeUrlAndReloadGrid(id,params,newParams){
	// alert("111");
	let newUrl='/waggle/service/report/data?id='+id+params+newParams;
...
function showPDFExportButton(id,params){
	...
	jQuery("#report_"+id).jqGrid('navButtonAdd', 
            ...
            onClickButton: function(){window.open('/waggle/service/report/writepdf?id='+id+params);}, position:"last",id:"pdfbutton_"+id});
}
function showCSVExportButton(id,params){
	...
	jQuery("#report_"+id).jqGrid('navButtonAdd', 
            ...
            title:"Export to CSV", 
            buttonicon:"csv-icon", 
            onClickButton: function(){window.open('/waggle/service/report/writecsv?id='+id+params);}, position:"last",id:"csvbutton_"+id});
}