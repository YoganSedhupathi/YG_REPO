...
(function ($) {
  ...
  function FlexTable(id,viewWidth,options,rowProperties,columnProperties,cellProperties) {
    ...
    this.onInputClick=function(e){
     ...
     this.onTextKeyDown=function(e,args){
    	...
    	if(key==13){
    		...
    		me.checkAndMove("Down",false); //for others
	...
	this.addRow=function(){
		...
		r.appendTo(wTable);
		rows.push(r);
		if(options.serialNumberColumn!=""){
			me.setText(s,me.getCols()[options.serialNumberColumn],s+1);
		}
	}    
	...
	this.generateSerialNo=function(){
		if(options.serialNumberColumn!=""){
			var s=me.getSize();
			for(var i=0;i<s;i++){
				me.setText(i,me.getCols()[options.serialNumberColumn],i+1);
			}
		}
	}	
	...
	this.checkAndMoveToPreviousCell=function(tmpCurrentRow,tmpCurrentCol){
		...
		if(!moved && tmpCurrentCol==0){
			for(row=tmpCurrentRow;row>0;row--){
				tmpCurrentRow--;
				if(tmpCurrentRow>=0 && me.checkRowEnabled(tmpCurrentRow)){
					if(FlexUI.trigger(me.onLeaveRow,{"name":name,"row":currentRow,"col":currentCol,"table":me},null,this)){
						for(col=me.getRow(tmpCurrentRow).getSize()-1;col>=0;col--){							
							tmpCurrentCol=col;						
							if(me.checkColumnEnabled(tmpCurrentRow,tmpCurrentCol)){
								me.moveWidget(tmpCurrentRow,tmpCurrentCol);
								return;
							}
						}
	...
	this.getValidRowSize=function(){
		...
		if(z>1){
			z=z-1;
			me.setCellPopup(z,me.getCols()[options.rowEmptyCheckColumn],false);			
			var b=me.validateCell(z,me.getCols()[options.rowEmptyCheckColumn]);
			if(b){
				z++;
			}			
		}else if(z==1 && options.tableIsOptional){
			me.setCellPopup(z-1,me.getCols()[options.rowEmptyCheckColumn],false);			
			var b=me.validateCell(z-1,me.getCols()[options.rowEmptyCheckColumn]);
			if(!b){
				z--;
			}						
		}
	...
	this.getObjectData=function(lastnoOfRows){
		var tableData={};
		var s=columnProperties.length;
		var z=me.getValidRowSize();
		var beginRow=0;
		if(lastnoOfRows!=undefined){
			beginRow=z-lastnoOfRows;
			if(beginRow<0){
				beginRow=0;
			}
		}
		tableData=[];		
		for(var i=beginRow;i<z;i++){
		    var row={};
			for(var j=0;j<s;j++){
				if(columnProperties[j]!=null){
					var saveTag=columnProperties[j].saveTag;
					...
					row[columnProperties[j].field]=escape(value);
				}
			}
			tableData.push(row);
		}
		return '"'+id+'":'+JSON.stringify(tableData);
	}	
	...
	this.setObjectData=function(tData){
		var cs=columnProperties.length;
		var ds=tData.length; 
		for(var i=0;i<ds;i++){
			var r=tData[i];
			...
			for(var j=0;j<cs;j++){				
				var c=r[columnProperties[j].field];
				if(c!=undefined && c.v){
					me.setText(i,j,c.v);
					me.setCellTag(i,j,c.t);
					if(c.r){
	...
	this.setObjectRowData=function(row,rData){
		var cs=columnProperties.length;
		for(var j=0;j<cs;j++){
			var c=rData[columnProperties[j].field];
			if(c.v){
	...
	this.clearRowData=function(row){
		var cs=columnProperties.length;
		for(var j=0;j<cs;j++){
			if(me.getCols()[options.serialNumberColumn]!=j){
				me.setText(row,j,"");
				me.setCellTag(row,j,"");
  ...
  function FlexCell(parent,columnProp,cellProp) {
    ...
    this.createWidget=function(columnProp,cellProp){
    	...
    	if(columnProp!=null){ 
    		...
    		}else if(columnProp.inputType=='3'){
    			btn=new FlexUI.Button("",row.getTable(),columnProp);
    			wCell.append(btn.getButton());
    		}
    	...
    	if(cellProp!=null){
    		if(cellProp.inputType=='1'){
    			...
    			wCell.append(check.getCheckBox());
    		}else if(cellProp.inputType=='3'){
    			btn=new FlexUI.Button("",row.getTable(),cellProp);
    			wCell.append(btn.getButton());
    		}
    	...
    	if(btn!=null){
    		btn.onClick.subscribe(row.getTable().onInputClick);
    		wRef=btn;
    	}
    }
	this.getCell=function(col){
