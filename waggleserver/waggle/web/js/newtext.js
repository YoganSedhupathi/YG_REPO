...
(function ($) {
  ...
  function FlexInput(id,parent,attributes) {
    ...
    this.init = function () {  
	  ...
	  wButton = $("<INPUT type=button  id='"+id+"' value='Select' title='"+attributes.tooltip+"'/>");      
	  if(attributes.type=='TEXTAREA'){
	  	...
	  	if($('#'+id).length==0){
	  	   wTextBox = $("<INPUT type=text id='"+id+"'  title='"+attributes.tooltip+"' class='form-control' data-toggle='tooltip' data-placement='bottom' autocomplete='off'/>");	  	
		}else{
		   wTextBox=$('#'+id);
		}	  	
	  }	      		
	...
	function handleKeyPress(e){
		...
		if(type=="TEXT" || type=="LOV"){
			...
			checkDateTime(e,key);
		}else if(type=="DATETIMEPICK"){
			checkDateTimepick(e,key);
		}
	...
	function handleClick(e){
		e.stopPropagation();
		var invokeLov=true;
		if(lovOptions!=undefined && lovOptions.invokeLovOnClick!=undefined){
			invokeLov=lovOptions.invokeLovOnClick;
		}
		if(invokeLov){
    ...
    this.validateAndGotoNextField=function(){
    	me.validate();
    	me.goToNextField();
    }    
    this.reset=function(){
    	...
    	me.setTag("");
    	if(type=='DATE'){
			wInput.datepicker({useCurrent: false ,changeMonth: true,changeYear: true, format:format});			
			if(wInput.val()==""){
				var cd=new Date();
				wInput.datepicker("setDate",cd);
			}
			wInput.datepicker("option",{onClose:me.validateAndGotoNextField});
		}
		else if(type=='DATETIMEPICK'){
			wInput.datepicker({useCurrent: true ,changeMonth: true,changeYear: true, format:format});			
			if(wInput.val()==""){
				var cd=new Date();
				wInput.datepicker("setDate",cd);
			}
	...
	this.setAttributes=function(attributes){
		...
		if(type=='DATE'){
			...
			wInput.datepicker({changeMonth: true,changeYear: true, format:format});			
			if(wInput.val()==""){
				var cd=new Date();
				wInput.datepicker("setDate",cd);
			}
			...
			wInput.datepicker({changeMonth: true,changeYear: true, 
				timeFormat: 'hh:mm TT',
				dateFormat:'dd/mm/yy',
				format:format
			});
...
function FlexSelect(id,parent,attributes) {
	...
	this.setOptions=function(attribs){
		...
		if(nextFocus==undefined || nextFocus==""){
