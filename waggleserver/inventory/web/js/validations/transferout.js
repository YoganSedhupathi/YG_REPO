...
function save(id,me){
	...
	if("FAILED"!=d){
		...
		showLoader();
		let r=s.callAjax("POST","/inventory/service/transferout",d);
		hideLoader();
		if(r.result=="Success"){
...
function delete1(id,me){
	...
	let l={url:"/inventory/service/ajax",ajax:"transferoutedit",scrollEnabled:true,searchFromList:false,dsi:2,params:[{n:"branch",src:"branch",ft:true},{n:"location",src:"location",ft:true}],results:[{tgt:"code",id:"0",tt:false}],closeFunction:"onEscClose"};
...
function onEditClose(e,args){	
	...
	showLoader();
	let r=a.callAjax("GET","/inventory/service/transferout","transferin=false&outno="+args.result[0]+"&branch="+branch.getTag());
	add('','');
	toscreen.setOperation("EDIT");
	toscreen.loadData(r);
	calculateTotal();
...
function loadDataView(no){	
	...
	let a=new FlexUI.Ajax();
	showLoader();
	let r=a.callAjax("GET","/inventory/service/transferout","transferin=false&outno="+no+"&branch="+branch.getTag());
