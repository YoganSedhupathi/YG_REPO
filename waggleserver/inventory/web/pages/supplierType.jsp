...
<div class="col-sm-10 col-md-10 col-lg-10 mpZero"><h1>Supplier Type</h1><ol class="breadcrumb mpZero"><li><a href="#">Home</a></li><li><a>Master</a></li><li class="active breaCSel"><strong>Supplier Type</strong></li></ol></div>
...
<script>
    ...
    var sProps={title:{height:30,width:100,name:"AMC"},container:{id:"callback",isdialog:false},js:"",screen:[
  	{name:"group1",title:"Supplier Type Details",height:40,width:98,minWidth:500,minHeight:200,colWidth:98,float:"left",backColor:"beige",contentType:"Fields",
  ...
  var screenProps=changeScreenAttributes(110015,sProps);
  var suppliertypem=new FlexUI.Screen("supptype",screenProps);
  jqReport(110015,"#xyz");
  function addMasterData(){ 
  	suppliertypem.clearScreen(); 
  ...
  function toggleHeaderRow(){
  	toggleSearchToolBar("110015");
  }
  toggleHeaderRow();
  function onSaveClicked(id,me){
    var code=screenObjects.getScreenObject("code").getValue();
    if(code!="0" && code!=""){
    	suppliertypem.setOperation("EDIT");
    }
	var d=suppliertypem.processDataToSave();
	if("FAILED"!=d){
		var s=new FlexUI.Ajax();
		showLoader();
		var r=s.callAjax("POST","/inventory/service/supplier/suppliertype",d);
		hideLoader();
		if(r.result=="Success"){
  ...
  function loadOrderData(url,code,did){
  	if(url==""){
  		url="/inventory/service/supplier/suppliertype";
  	}
  	if(code!=""){
  		code="code="+code+"";
  	}
