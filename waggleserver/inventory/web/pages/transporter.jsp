...
<script>
    ...
    var sProps={title:{height:30,width:100,name:"AMC"},container:{id:"callback",isdialog:false},js:"",screen:[
  	...
  	{colno:0,name:"branch",inputType:0,type:"LOV",caption:"Branch",mandatory:false,defaultValue:"0","maxValue":0,"textLength":10,"decimalLength":0,"visible":false,"readOnly":false,"chkTag":false,nf:"name",lovOptions:{url:"/inventory/service/ajax",ajax:"userbranch",scrollEnabled:true,searchFromList:false,dsi:0,params:[],results:[]}},
  ...
  var transporterm=new FlexUI.Screen("transporter",screenProps);
  jqReport(28,"#xyz");
  ...
  function toggleHeaderRow(){
  	toggleSearchToolBar("28");
  }
  toggleHeaderRow();
  function onSaveClicked(id,me){
    var code=screenObjects.getScreenObject("code").getValue();
    if(code!="0" && code!=""){
    	transporterm.setOperation("EDIT");
    }
	var d=transporterm.processDataToSave();
	if("FAILED"!=d){
		var s=new FlexUI.Ajax();
		showLoader();
		var r=s.callAjax("POST","/inventory/service/transporter",d);
		hideLoader();
		if(r.result=="Success"){
