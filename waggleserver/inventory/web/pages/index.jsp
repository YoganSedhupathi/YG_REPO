...
<div id="errorContainer" style='display:none;z-index:1001;' class="errorContainer">
	<div id="error" class="error">
		<span class="errorContent"><b id='error-message'> check </b><a id='errorClose' onclick='closeError()' class='close'><span class="ml-2 mdi mdi-close"></span></a></span>
	</div>
</div>
	
<!-- <div class="modal" tabindex="-1" id="logoutmodal" style="display: none;">
	<div class="modal-dialog">
	  <div class="modal-content">
		<div class="modal-header">
		  <h4 class="modal-title">Logout</h4>
		  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"  onclick="logNo()"></button>
		</div>
		<div class="modal-body">
		  <h5>Do you want to exit the application?</h5>
		</div>
		<div class="modal-footer">
		  <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" onclick="logNo()">No</button>
		  <button type="button" class="btn btn-primary"  onclick="logYes()">Yes</button>
		</div>
	  </div>
	</div>
  </div> -->
 ...
 var menuData=ajaxObj.callAjax("GET","/inventory/service/ajax","ic=0&id=usermenu&login_id=<%=userId%>");
				...
				if(m!=null && m!="" && m!="#"){
					...
					if(menuId==1010001){
						if(window.confirm("Do you want to exit the application?")){
							$("#bodyContainer").html("");
							$("#bodyContainer").load("/inventory"+m,onSuccess);
						...
						window.open("/inventory"+m,"touch","fullscreen=yes,location=no,menubar=no,titlebar=no,toolbar=no");
					}
				}
				//showSideMenu(2);
			}
