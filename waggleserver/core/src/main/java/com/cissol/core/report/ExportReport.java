...
import com.cissol.core.report.JasperReport.PageSizeEnum;
...
public class ExportReport {
	...
	public static void exportReport(HttpServletResponse response, OutputStream out,
			MultivaluedMap<String, String> parameters, String type) {
		Connection con = null;
		...
		String broughtForwardText = "B/F";
		String reportQuery = "";
		String openingBalanceQuery = "";
		boolean printOpeningBalance = false;
		boolean printBroughtForward = false;
		boolean printClosingBalance = false;
		String queryParams = "";
		String headerQuery = "";
		String reportTitle = "";
		String reportName = "";
		boolean printTitleInAllPages = false;
		...
		try {
			...
			if (rs.next()) {
				...
				printOpeningBalance = rs.getBoolean("opening_balance");
				printBroughtForward = rs.getBoolean("brought_forward");
				printClosingBalance = rs.getBoolean("print_closing");
				queryParams = rs.getString("parameters");
				broughtForwardText = rs.getString("brought_fwdtext");
				printTitleInAllPages = rs.getBoolean("title_for_all");
				bfColumn = rs.getString("bf_column");
			...
			if (rs.next()) {
				...
				titles.put("title_length", String.valueOf(s + 1));
				for (int i = 1; i <= s; i++) {
					titles.put("title_" + (i), rs.getString(i));
				}
				titles.put("title_" + (s + 1), reportTitle);
			}
			rs.close();
			stmt.close();
			HashMap<String, String> openingBalances = new HashMap<String, String>();
			if (printOpeningBalance) {
				stmt = con.createStatement();
				rs = stmt.executeQuery(openingBalanceQuery);
				if (rs.next()) {
					ResultSetMetaData md = rs.getMetaData();
					int s = md.getColumnCount();
					for (int i = 1; i <= s; i++) {
						openingBalance = rs.getDouble(i);
						openingBalances.put(md.getColumnLabel(i).toUpperCase(), String.valueOf(openingBalance));
					}
				}
				rs.close();
				stmt.close();
			}
			openingBalances.put("TEXT_COLUMN", bfColumn);
			openingBalances.put("BF_TEXT", broughtForwardText);
			int noOfRows = 0;
			...
			JasperReport j = new JasperReport(rs.getMetaData());
			j.setLinkServerName(parameters.getFirst("server"));
			j.setType(type);
			j.setNumberOfRows(noOfRows);
			j.pageSetup(PageSizeEnum.A4, OrientationEnum.PORTRAIT);
			j.addFields();
			j.addTitle(titles, printTitleInAllPages);
			j.addColumnHeader(columns);
			j.addDetail(columns);
			j.addVariables(columns, openingBalances);
			j.addColumnFooter(columns, openingBalances);
			j.setOpeningBalance(columns, printOpeningBalance, openingBalances);
			j.addLastPageColumnFooter(columns, openingBalances);
			j.addSummary(columns, openingBalances);
			JasperDesign jd = j.getDesign();
			...
			if ("pdf".equals(type)) {
				...
				response.setHeader("content-disposition", "inline;filename=" + reportTitle + ".xls");
...
//            exporter.setParameter(JRExporterParameter.JASPER_PRINT, jp);
//            exporter.setParameter(JRHtmlExporterParameter.IMAGES_URI, "/jrimage?image=");
//            Map imagesMap = new HashMap();
//            exporter.setParameter(JRHtmlExporterParameter.IMAGES_MAP, imagesMap);
//            exporter.setParameter(JRHtmlExporterParameter.IS_WHITE_PAGE_BACKGROUND, false);
//            exporter.setParameter(JRHtmlExporterParameter.IS_USING_IMAGES_TO_ALIGN, false);
//            response.setHeader("content-disposition", "inline;filename=" + reportTitle+ ".html");
//    		exporter.setParameter(JRExporterParameter.OUTPUT_STREAM, out);        
//            exporter.exportReport();
//        }
		} catch (Exception e) {
