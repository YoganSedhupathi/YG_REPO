...
public class DatabaseUtil {
	...
	public static String getTenantConnectionUrl(String domainName) throws Exception {
		Connection dbConnection = null;
		String query = "SELECT  rt_conn_url,rt_tenant_id from registered_tenants where rt_tenant_domain='" + domainName
				+ "'";
		Statement stmt = null;
		ResultSet rs = null;
		String tenantDatabaseUrl = "";
		try {
			System.out.println(domainName);
			dbConnection = getConnection("jdbc/mydb");
			stmt = dbConnection.createStatement();
			rs = stmt.executeQuery(query);
			if (rs.next()) {
				String connUrl = rs.getString("rt_conn_url");
				tenantDatabaseUrl = connUrl;
			} else {
				System.out.println(" Tenant Url not found ");
			}
		} catch (Exception ex) {
			throw ex;
		} finally {
	...
	public static Connection getConnection() throws Exception {
		...
		if (InitProjectServlet.isMultiTenant) {
			TenantInfo info = CurrentTenantUtil.getTenantInfo();
			String url = info.getTenantDatabaseUrl();
			if (driver == null) {
			...
			if ("".equals(url)) {
				url = getTenantConnectionUrl(info.getTenantDomain());
			}
	...
	public static Connection getMultiTenantConnection(String domainName) throws Exception {
		Connection con = null;
		if (InitProjectServlet.isMultiTenant) {
			String tenantUrl = getTenantConnectionUrl(domainName);
			if (driver == null) {
				Class<?> c = Class.forName(driverName);
				driver = (Driver) c.getDeclaredConstructor().newInstance();
			}
			Properties p = new Properties();
			con = driver.connect(tenantUrl, p);
		}
		return con;
	}
